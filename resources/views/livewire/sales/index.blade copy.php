...
<div class="p-6" x-data x-on:precio-updated.window="$dispatch('input')" class="p-6">
    ...
    <div class="mb-4 flex gap-4 items-end">
        ...
        <button wire:click="startCreatingSale" class="bg-amber-100 text-black p-2 rounded-sm cursor-pointer">Registrar
            venta</button>
    </div>
        ...
        <table class="min-w-full border border-gray-700 text-sm text-gray-300">
            ...
            <tbody>
                @forelse ($sales as $sale)
                    <tr class="hover:bg-gray-800">
                        ...
                        <td class="px-2 py-1 border text-right">
                            {{ $this->sanitizeForJson(number_format($sale->precio_kg, 2)) }}</td>
                        <td class="px-2 py-1 border text-right">
                            {{ $this->sanitizeForJson(number_format($sale->total, 2)) }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-2 py-1 border text-center">No hay ventas registradas para este
                            período.</td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot class="bg-gray-800 text-gray-200 font-bold">
                <tr>
                    ...
                    <td class="px-2 py-1 border text-right">
                        {{ $this->sanitizeForJson(number_format($sales->sum('kgs'), 3)) }}</td>
                    <td class="px-2 py-1 border text-center">-</td>
                    <td class="px-2 py-1 border text-right">
                        {{ $this->sanitizeForJson(number_format($sales->sum('total'), 2)) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    @if ($creatingSale)
        ...
        <div class="mb-6 overflow-x-auto">
            ...
            <label class="block text-sm font-medium mt-4">Precio/Kg:</label>
            <div x-data="{ initial: @js($precio_kg) ?? 0 }" x-init="initPrecioCleave()">
            ...
            <div class="mt-4 flex gap-4">
                <button type="button" wire:click="confirmSave"
                    class="bg-green-500 text-white p-2 rounded-sm cursor-pointer">Guardar Venta</button>
                <button wire:click="cancelCreatingSale"
                    class="bg-red-500 text-white p-2 rounded-sm cursor-pointer">Cancelar</button>
            </div>
        </div>
        ...
        return function() {
            ...
            input.cleave = new Cleave(input, {
                ...
                onValueChanged: function(e) {
                    ...
                    // === DISPARA REACTIVIDAD EN EL FRONTEND ===
                    @this.dispatch('precio-updated');
                }
            });
